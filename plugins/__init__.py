...
@stark.on_message(filters.command(["start"]) & ~filters.user(AUTH_USERS))
...
@stark.on_message(filters.command(["restart"]) & ~filters.user(AUTH_USERS))
async def restart_handler(_, m):
    await m.reply_text("Restarted!", True)
    os.execl(sys.executable, sys.executable, *sys.argv)

@stark.on_message(filters.command(["log"]) & ~filters.user(AUTH_USERS))
